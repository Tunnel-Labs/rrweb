import { EventType, eventWithTime, RecordPlugin } from '@rrweb/types';
...
describe('record integration tests', function(this: ISuite) {
				...
				visitSnapshot(event.data.node, (n) => {
					if (n.type === NodeType.Element && n.attributes.rr_dataURL) {
						n.attributes.rr_dataURL =
							`LOOKS LIKE WE COULD NOT GET STABLE BASE64 FROM SAME IMAGE.`;
					}
				});
			}
		}
	...
	it('should record console messages', async () => {
		...
		await page.setContent(
			getHtml('log.html', {
				plugins:
					'[rrwebConsoleRecord.getRecordConsolePlugin()]' as unknown as RecordPlugin<
						unknown
					>[],
			}),
		);
