...
import { getServerURL, startServer } from './utils.js';
	...
	await frame.evaluate(
		(rrwebCode, pluginCode) => {
			...
			(async () => {
				...
				window.plugin = new rrwebCanvasWebRTCRecord
...
void (async () => {
	...
	async function record(url) {
		...
		replayerPage.on(
			'console',
			(msg) => console.log('REPLAY PAGE LOG:', msg.text()),
		);
		recordedPage.on(
			'console',
			(msg) => console.log('RECORD PAGE LOG:', msg.text()),
		);
