...
import type { missingNodeMap, playerConfig } from '../types/_.js';
import {
	AppendedIframe,
	getBaseDimension,
	getNestedRule,
...
import { createPlayerService, createSpeedService } from './machine.ts';
import { polyfill as smoothscrollPolyfill } from './smoothscroll.ts';
import getInjectStyleRules from './styles/inject-style.ts';
import { Timer } from './timer.ts';
import './styles/style.css';
import canvasMutation from './canvas/index.ts';
import { deserializeArg } from './canvas/deserialize-args.ts';
...
export class Replayer {
	...
	private async preloadAllImages(): Promise<void[]> {
		...
		for (const event of this.service.state.context.events) {
			if (
				...
				const commands = 'commands' in event.data ?
					event.data.commands :
					[event.data];
				commands.forEach((c) => {
	...
	private applyStyleSheetMutation(
		...
		if (data.styleId) styleSheet = this.styleMirror.getStyle(data.styleId);
			...
			styleSheet = (this.mirror.getNode(data.id) as HTMLStyleElement)?.sheet ||
				null;
		}
